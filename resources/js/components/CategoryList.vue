    ...
    <div>
        <div class="w-full font-poppins overflow-x-scroll noscroll">
            <table class="w-full min-w-[600px] border-separate border-spacing-y-4">
              ...
              <tbody>
                  <tr v-for="(item, index) in products.slice(start, end)" :key="item.id" class="*:py-4 *:px-4 font-[500] text-[14px]">
                    <td class="font-[600]">{{ index+1 }}</td>
                    <td>{{ item.name }}</td>
                    <td>{{ item.updated_at.split('-')[0]+'-'+item.updated_at.split('-')[1]+'-'+item.updated_at.split('-')[2].slice(0,2) }}</td>
                    <td>
                        <div class="flex items-center">
                            <a :href="'/categories/edit/'+item.id" class="text-[#262b45] block font-[600] cursor-pointer text-[13px] ml-1"><i class="fa-solid fa-pen ml-2"></i>تعديل</a>
...
//   import HeaderAdmin from '../components/HeaderAdmin.vue'
  ...
  const deletec = (itemId) => {
      fetch(`/categories/delete/${itemId}`, {
          method: 'DELETE',
          headers: {
