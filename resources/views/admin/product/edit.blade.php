...
<div id="app" class="mr-auto sm:w-[calc(100%-40px)] md:w-[calc(100%-250px)] bg-[white] h-fit min-h-screen px-[50px] pt-[30px]">
    ...
    <div class="mt-[50px] mb-[20px] w-fit cursor-pointer">
        <h1 class="text-[14px] font-[600] w-fit">تحديث خدمة</h1>
        <div class="w-full h-[2px] bg-black rounded-lg"></div>
    </div>

    <form action="{{ url('/products/edit/'.$product->id) }}" method="POST" enctype="multipart/form-data" class="w-full h-fit block rounded-md mt-[60px] px-[40px] pb-[60px]">
        @csrf
        @method('PUT')
        <h1 class="text-[20px] font-[600] text-[#2f3334]">تحديث خدمة</h1>
        ...
        <div class="flex items-center gap-x-4">
            <div class="flex items-center">
                <input id="default-radio-1" type="radio" value="منتوج" @if($product->type == 'منتوج') checked @endif name="type" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                <label for="default-radio-1" class="ms-2 text-sm font-medium text-gray-900">منتوج</label>
            </div>
            <div class="flex items-center">
                <input id="default-radio-2" type="radio" value="خدمة" @if($product->type == 'خدمة') checked @endif name="type" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
        ...
        <img class="w-[100px] h-[100px] rounded-md object-cover" id="previewImage" src="{{url($product->cover)}}"/>

        <div class="my-4">
          <label for="" class="text-[14px]">الاسم</label>
          <input type="text" name="name" value="{{ $product->name }}" class="w-full border h-fit text-[15px] px-4 py-2 rounded-md bg-[#f0f0f041] mt-2 outline-none">
        </div>
        <div class="my-4">
            <label for="" class="text-[14px]">الثمن</label>
            <input type="text" name="price" value="{{ $product->price }}" class="w-full border h-fit text-[15px] px-4 py-2 rounded-md bg-[#f0f0f041] mt-2 outline-none">
        </div>
        <div class="my-4">
            ...
            <select name="category[]" multiple class="w-full border h-fit text-[15px] px-4 py-2 rounded-md bg-[#f0f0f041] mt-2 outline-none">
                @foreach($categories as $item)
                    <option value="{{ $item->id }}" @if(collect($product->categories)->pluck('id')->contains($item->id)) selected @endif>{{ $item->name }}</option>
                @endforeach
            </select>
        </div>
